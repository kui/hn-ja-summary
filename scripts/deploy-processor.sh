...
#   GEMINI_API_KEY, CLOUDFLARE_ACCOUNT_ID, CLOUDFLARE_D1_DATABASE_ID
#   CLOUDFLARE_API_TOKEN, MAX_COMMENTS
# Optional env vars:
...
: "${CLOUDFLARE_D1_DATABASE_ID:?Set CLOUDFLARE_D1_DATABASE_ID}"
...
gcloud run deploy hn-feed-processor \
  ...
  --set-env-vars="GEMINI_API_KEY=${GEMINI_API_KEY},CLOUDFLARE_ACCOUNT_ID=${CLOUDFLARE_ACCOUNT_ID},CLOUDFLARE_D1_DATABASE_ID=${CLOUDFLARE_D1_DATABASE_ID},CLOUDFLARE_API_TOKEN=${CLOUDFLARE_API_TOKEN},GCP_PROJECT_ID=${PROJECT_ID},MAX_COMMENTS=${MAX_COMMENTS}" \
  --concurrency=10 \
  --cpu=1 \
  --memory=512Mi \
  --timeout=300 \
