...
<p>{{HNコメントの総括。記事の文体・AI生成の疑い・paywall・タイトルの釣りなど、記事の内容そのものではないメタな指摘はここで1文に集約し、おおよその割合を添える（例:「コメントの約3割は記事のAI生成調の文体への批判」）。}}</p>

{{以下では主要な観点・議論の軸ごとにまとめてその数だけ繰り返す。観点は記事の内容に関する議論のみで構成し、メタな指摘は観点として立てない。内容に関する実質的な議論が乏しい場合は観点を無理に増やさず、その旨を総括に記載。返信の多いコメントがリンクを含んでいるときは必ずそのリンクを含めて出力。投稿に含まれていないURLをリンクすることは厳禁。}}
