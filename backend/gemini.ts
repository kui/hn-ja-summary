...
<p>{{記事の内容・背景・意義を3〜5文で簡潔に説明。必要に応じてtable要素やol,ul要素など構造化などを利用。HNコメントからの補完は厳禁。HNコメントからの内容の混合は厳禁。取得失敗、取得スキップ、内容が不十分の時にはその旨のみを記載。}}</p>

<h2>HNコミュニティの反応</h2>
<ul><li><a href="${hnUrl}">${hnUrl}</a></li></ul>
<p>{{HNコメントの総括}}</p>

{{以下では主要な観点・議論の軸をまとめてその数だけ繰り返す。返信の多いコメントがリンクを含んでいるときは必ずそのリンクを含めて出力。投稿に含まれていないURLをリンクすることは厳禁。}}
